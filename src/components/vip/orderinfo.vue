                    ...
                    <div class="comp">
                        <div class="tmpl routeanimate">
                            ...
                            <div class="section clearfix">
                                ...
                                <div class="right-auto" v-if="info.goodslist">
                                        ...
                                        <div class="form-box accept-box form-box1">
                                            <dl class="head form-group">
                                                <dd>
                                                    ...
                                                    <router-link v-if="info.orderinfo.status==1" v-bind="{to:'/site/pay'+info.orderinfo.id}" class="btn-pay">去付款</router-link>
                                                    <!---->
                                                    <a v-if="info.orderinfo.status==3" href="jacascript:void(0)" class="btn-pay" @click="complate">签收</a>
                                                    <a v-if="info.orderinfo.status==4" href="jacascript:void(0)" class="btn-pay" >已完成</a>
                                                </dd>
                                            </dl>
                                            <dl class="form-group">
                                                <dt>订单状态：</dt>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr v-for="item in info.goodslist" :key="item.id">
                                                        <td width="60">
                                                            <router-link v-bind="{to:'/site/goodsinfo'+item.goods_id}">
                                                                <img :src="item.imgurl" class="img  shoppingimg">
                                                            </router-link>
                                                        </td>
                                                        <td align="left">
                                                            <router-link v-bind="{to:'/site/goodsinfo'+item.goods_id}">
                                                                <a target="_blank" href="javascript:void(0)">{{item.goods_title}}</a>
                                                            </router-link>
                                                        </td>
                                                        <td align="center">
                                                    ...
                                                    <tr>
                                                        <td colspan="7" align="right">
                                                            <p>商品金额：
                                                                <b class="red">￥{{ordertotalcount}}</b>&nbsp;&nbsp;+&nbsp;&nbsp;运费：
                                                                <b class="red">￥{{info.orderinfo.express_fee}}</b>
                                                            </p>
                                                            <p style="font-size: 22px;">应付总金额：
                                                                <b class="red">￥{{ordertotalcount+info.orderinfo.express_fee}}</b>
                                                            </p>
                                                        </td>
                                                    </tr>
                                                </tbody>
            ...
            getorderinfo() {
                ...
                this.$ajax.get(url).then(res => {
                    ...
                    this.info.goodslist.forEach((item) => {
                        this.ordertotalcount += (item.real_price * item.quantity);
                    });
                })
            },
            complate(){
                var url = '/site/validate/order/complate/'+this.info.orderinfo.id;
                this.$ajax.get(url).then(res=>{
                    if(res.data.status){
